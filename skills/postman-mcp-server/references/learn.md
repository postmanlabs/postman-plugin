...
The mode is fixed by the endpoint in each route's MCP config, not by the environment. Routes pinned to `https://mcp.postman.com/mcp` (Claude Code, Cursor) have the tool; routes pinned to `https://mcp.postman.com/minimal` (Kimi Code, opencode) do not. **`POSTMAN_MCP_MODE` is not read on any route** — never tell the user to set or unset it to change the tool set.
...
- **`searchLearningCenter` unavailable:** Confirm with `getEnabledTools`. Expected on a route pinned to the `minimal` endpoint (Kimi Code, opencode) — say the tool isn't in that route's tool set and point the user at https://learning.postman.com. On a `/mcp` route (Claude Code, Cursor): "Run `/postman:setup` to configure the Postman MCP Server."
